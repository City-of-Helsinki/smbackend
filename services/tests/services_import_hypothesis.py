...
def int_keys(draw):
    return draw(
        lists(integers(min_value=1, max_value=15), min_size=1, max_size=3, unique=True)
    )
...
MUNICIPALITIES = [  # TODO: read from independent source
    {"fi": "Helsinki", "sv": "Helsingfors", "en": "Helsinki"},
    {"fi": "Espoo", "sv": "Esbo", "en": "Espoo"},
    {"fi": "Kauniainen", "sv": "Grankulla", "en": "Kauniainen"},
    {"fi": "Vantaa", "sv": "Vanda", "en": "Vantaa"},
]
...
def accessibility_viewpoints(draw):
    return ",".join(
        [
            "{}:{}".format(key, draw(sampled_from(VIEWPOINT_STATES)))
...
def make_source(draw):
    return {
        "id": draw(text(SAFE_LETTERS, min_size=1, max_size=100)),
...
def add_extra_searchwords(draw, result):
    for lang in LANGUAGES:
        words = draw(sets(text(SAFE_LETTERS + "åäöÅÄÖ ", min_size=1, max_size=25)))
        if len(words) == 0:
...
def unit_maker(draw, resource_ids):
    def make_unit(uid):
        ...
        add_optional_field("sources", lists(make_source(), min_size=1, max_size=2))
...
def make_ontologyword_details(draw, unit_id, ontologyword_id):
    result = {"unit_id": unit_id, "ontologyword_id": ontologyword_id}
    if draw(booleans()):
...
def closed_object_set(draw):
    ...
    for unit in resources["unit"]:
        for oid in draw(permutations(ids["ontologyword"])):
            resources["ontologyword_details"].append(
                make_ontologyword_details(draw, unit["id"], oid)
            )
    return resources
