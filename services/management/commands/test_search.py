...
def get_git_commit(only_index=False, ignore_working_dir=False):
    ...
    if len(result.stdout) == 0 or ignore_working_dir:
        ...
        if only_index is False:
            ...
            command = restrict_to_index_if_desired(["git", "log", "-n 1", "--pretty=format:%h", "--"])
            result = subprocess.run(command,
                                    stdout=subprocess.PIPE,
                                    encoding='utf-8')
...
def get_relevant_commits():
    ...
    if current_commit is None:
        return {
            'new': None,
            'old': get_git_nth_commit(0)}
    else:
        return {
            'new': get_git_nth_commit(0),
            'old': get_git_nth_commit(1)
        }
...
def save_results(results):
    ...
    if commit is None:
        print('Running on working directory, results will not be saved until changes committed.')
        return
...
def format_difference(old_position, new_position, only_not_found=False, zero_symbol='!'):
...
def compare_results(commits, show_unchanged=False, no_input=False):
    ...
    for k in ['old', 'new']:
        if results[k] is None:
            print('''Cannot find persisted results for {0}
    ...
    for key in ['old', 'new']:
        for query in QUERIES['queries']:
            ...
            for suggestion in _results:
                suggestion_max_length = max(len(suggestion['suggestion']), suggestion_max_length)
    ...
    for query in QUERIES['queries']:
        ...
        for key in ['old', 'new']:
            if query not in results[key]['queries']:
                warnings.append(
                    '→ NO RESULTS FOUND IN FILE FOR COMMIT {} -- PLEASE RE-RUN TESTS THERE'.format(commits[key]))
        print(query, '→ RESULTS UNCHANGED BETWEEN REVISIONS' if unchanged else '',
              ' '.join(warnings))
        ...
        for suggestion_l, suggestion_r in zip_longest(new_suggestions, old_suggestions):
            ...
            if suggestion_l:
                old_position = next((
                    index for index, value in enumerate(old_suggestions)
                    if value['hash'] == suggestion_l['hash']), None)
            new_position = None
            if suggestion_r:
                new_position = next((
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('save', nargs='?', type=str,
