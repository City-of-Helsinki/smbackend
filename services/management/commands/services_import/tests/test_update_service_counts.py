...
def services():
    os = []
    for i in range(0, 5):
        o = Service.objects.create(name='service{}'.format(i), id=i, last_modified_time=now())
        o.save()
        os.append(o)
    return os
...
def units(services, municipalities):
    ...
    return {
        'units': units,
        'count_rows': len(distinct_service_muni_counts) + 1
    }
...
def test_update_service_counts(municipalities, services, units, api_client):
    ...
    for i in range(0, 10):
        ...
        u.save()
        UnitServiceDetails.objects.create(
            unit=u,
            service=service
        ).save()
        update_service_counts()
