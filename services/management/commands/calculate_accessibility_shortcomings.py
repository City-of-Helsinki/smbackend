...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--print-rules', action='store_true', help='Display rules only')
    ...
    def handle(self, **options):
        ...
        progress_bar = (tqdm(desc="Calculating shortcomings", total=Unit.objects.count())
    ...
    def print_rules(self):
        ...
        def print_rule(rule, indent=""):
            message = (' >> {}'.format(list(Calculator().messages[rule['msg']].values())[0])
                       if rule['msg'] is not None and rule['msg'] < len(Calculator().messages)
                       else '')
            try:
                evaluation = (' {} : {}'.format(AccessibilityVariable.objects.get(id=rule['operands'][0]),
                                                rule['operands'][1])
