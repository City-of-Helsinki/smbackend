...
from rest_framework import generics, renderers, serializers, status, viewsets
...
class UnitEntranceViewSet(munigeo_api.GeoModelAPIView, viewsets.ReadOnlyModelViewSet):
    ...
    def picture(self, request, pk=None):
        ...
        if not unit_entrance.picture_url:
            raise NotFound()
        headers = {"Location": unit_entrance.picture_url}
...
class UnitViewSet(
    ...
    def picture(self, request, pk=None):
        ...
        if not unit.picture_url:
            raise NotFound()
        headers = {"Location": unit.picture_url}
        return Response(status=status.HTTP_302_FOUND, headers=headers)
...
class AnnouncementViewSet(OutdoorSportsMapUsageViewSet):
    ...
    def picture(self, request, pk=None):
        ...
        if not announcement.picture_url:
            raise NotFound()
        headers = {"Location": announcement.picture_url}
        return Response(status=status.HTTP_302_FOUND, headers=headers)
...
class ErrorMessageViewSet(OutdoorSportsMapUsageViewSet):
    ...
    def picture(self, request, pk=None):
        ...
        if not error_message.picture_url:
            raise NotFound()
        headers = {"Location": error_message.picture_url}
        return Response(status=status.HTTP_302_FOUND, headers=headers)
