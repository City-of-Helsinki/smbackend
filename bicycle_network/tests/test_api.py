...
def test_bicycle_network(api_client, bicycle_network):
    ...
    assert results[0]["name"] == "main_network"
    assert results[1]["name"] == "local_network"
...
def test_bicycle_network_part(api_client, bicycle_network, bicycle_network_part):
    ...
    url = reverse("bicycle_network:bicycle_networkparts-list") + "?network_name=main_network"
    response = api_client.get(url)    
    # One part in main_network
    assert response.json()["count"] == 1
    # query with bbox including both parts
    url = reverse("bicycle_network:bicycle_networkparts-list") + "?bbox=22.0,59.8,22.3,60.4"
    response = api_client.get(url)    
    assert response.json()["count"] == 2
    # query with bbox including only one part 
    url = reverse("bicycle_network:bicycle_networkparts-list") + "?bbox=22.2,60.2,22.3,60.4"
    response = api_client.get(url)    
    assert response.json()["count"] == 1
    # query with bbox outside the generated lines
    url = reverse("bicycle_network:bicycle_networkparts-list") + "?bbox=22.4,60.5,22.5,60.7"
    response = api_client.get(url)    
    assert response.json()["count"] == 0
    # query for parts that are completly inside 1000m radius from point 22.25,60.3
    url = reverse("bicycle_network:bicycle_networkparts-list") + "?lon=22.25&lat=60.3&distance=1000"
    response = api_client.get(url)    
    assert response.json()["count"] == 1
    # query for parts that are completly inside 12000m radius from point 22.1,60.1
    url = reverse("bicycle_network:bicycle_networkparts-list") + "?lon=22.1&lat=60.1&distance=120000"
    response = api_client.get(url)    
    assert response.json()["count"] == 2
