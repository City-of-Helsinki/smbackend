...
def bicycle_network():
    main_netowrk = BicycleNetwork.objects.create(
        name="main_network", name_sv="huvudnätverk", length=4.2
    )
    local_network = BicycleNetwork.objects.create(name="local_network")
...
def bicycle_network_part(bicycle_network):
    ...
    bnp1 = BicycleNetworkPart.objects.create(
        bicycle_network=main_network, geometry=line_string
    )

    line_string = LineString(generate_coords(22.1, 60.1, 22.0, 59.8, 10))
    bnp2 = BicycleNetworkPart.objects.create(
        bicycle_network=local_network, geometry=line_string
    )
