...
class UnitPTVImporter:
    ...
    def _handle_location(self, unit_obj, unit_data):
        if unit_data["addresses"]:
            ...
            if addresses:
                # Coordinates
                latitude = addresses.get("latitude")
                longitude = addresses.get("longitude")
    ...
    def _handle_email_and_phone_numbers(self, unit_obj, unit_data):
        ...
        if numbers:
            if numbers[0].get("prefixNumber"):
                phone_number = numbers[0].get("prefixNumber") + numbers[0].get("number")
            else:
                phone_number = numbers[0].get("number")
            names = {}
    ...
    def _handle_service_ids(self, unit_data):
