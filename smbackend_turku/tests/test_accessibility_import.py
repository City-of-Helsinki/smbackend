...
from services.models import AccessibilityVariable, UnitAccessibilityProperty, UnitIdentifier, Unit
from smbackend_turku.tests.test_units_import import get_test_resource, create_municipality
...
def test_accessibility_variables_import(ar_resource, ar_se_resource, ar_se_accessibility_resource):
    ...
    ar_resource.return_value = get_test_resource(resource_name="accessibility/variables")
    ar_se_resource.return_value = get_test_resource(resource_name="info")
    ar_se_accessibility_resource.return_value = get_test_resource(resource_name="properties")
    ...
    assert accessibility_1.name == "ENTRANCE.DOOR.sufficiently_room_for_wheelchair_in_front"
    ...
    assert UnitAccessibilityProperty.objects.get(unit=unit_identifier_1.unit, variable_id=259)
    assert UnitAccessibilityProperty.objects.get(unit=unit_identifier_2.unit, variable_id=72)

    unit_accessibility_property_1 = UnitAccessibilityProperty.objects.get(unit=unit_identifier_1.unit, variable_id=259)
    unit_accessibility_property_2 = UnitAccessibilityProperty.objects.get(unit=unit_identifier_2.unit, variable_id=72)
